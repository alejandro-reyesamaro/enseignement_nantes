...
int proba(double p){
	...
	double q;
	for (i=0;i<25;i++){
		q=frand_a_b(0,1); //double aleatoire entre 0 et 1
		if (q<p){
			s++;
		}
	}
...
int main(int argc, char *argv[])
{
	...
	sem_init(&mutex, 0, 0); //initialisation a 0 car comme ça chasseur et cerf ne peuvent pas agir tant que l'inspecteur a pas libere qui sera le seul a pouvoir le faire au premier mois
