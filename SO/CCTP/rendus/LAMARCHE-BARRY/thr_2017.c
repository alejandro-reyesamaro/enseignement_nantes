...
// Déclaration du sémaphore d'exclusion mutuelle
sem_t inspec_d;
sem_t inspec_h;
...
void * code_thread_deer(void * arg)
{
	...
	{	
		...
		sleep(2);
		sem_post(&inspec_d);  /* V(s); */
		sem_post(&inspec_h);  /* V(s); */
	}
	return NULL;
}
...
int main(int argc, char *argv[])
{
	...
	sem_init(&inspec_d, 0,2);
	sem_init(&inspec_h, 0,2);
	...
	sem_destroy(&inspec_d);
	sem_destroy(&inspec_h);
