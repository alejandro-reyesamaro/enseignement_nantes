/* BOUDINE Alexandre, SALAUN Nathan*/
/* Utilisation de GLPK en mode bibliothèque */
/* Il s'agit de l'exercice 2.4 des feuilles de TD, qui a servi à illustrer l'utilisation d'une matrice creuse avec GNUMathProg */
...
{	
	...
	int *ia; //lignes
	int *ja; //colonnes
	double *ar; // valeurs - déclaration des 3 tableaux servant à définir la matrice "creuse" des contraintes
	...
	glp_set_prob_name(prob, "td2-4"); /* affectation d'un nom (on pourrait mettre NULL) */
	...
	for(i=1;i<=p.nbvar;i++)
	{
		...
		glp_set_col_name(prob, i , nomvar[i-1]); /* Les variables sont nommées "x1", "x2"... afin de respecter les noms de variables de l'exercice 2.4 */
	...
	glp_write_lp(prob,NULL,"td2-4.lp");
	...
	for(i = 0;i < p.nbvar;i++) 
	{
	    printf("x%d = %d   ",i+1,(int)(x[i] + 0.5)); /* un cast est ajouté, x[i] pourrait être égal à 0.99999... */    
	    if (i%2 == 1) printf("\n");
	}
	printf("z = %lf\n",z);
	puts("");
